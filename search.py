...
class NodePriorityQueue:
    ...
    def __init__(self, heuristic_function, values=[]):
        self.heuristic = heuristic_function
        self.values = values
        heapq.heapify(self.values)

    def push(self, x):
        """if an instance of Node is pushed onto the queue, hueristic(node) is used to prioritize the queue"""
        if isinstance(x,Node):
            heapq.heappush(self.values, (self.heuristic(x), x))
        else:
    ...
    def does_contain(self, _node):
        ...
        for i in range(len(self.values)):
            if _node.state in self.values[i]:
                return True

        return False

    def update(self, _node):
        """Updates the path cost of a given node."""
        self.values[self.get_index(_node)][0] = copy.copy(_node.path_cost)
        heapq.heapify(self.values)
    
    def get_index(self, _node):
        """Gets the index of a node."""

        if self.does_contain(_node):
            for i in range(len(self.values)):
                if _node.state in self.values[i]:
                    return i
        else:
